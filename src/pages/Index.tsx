...
const POEMS = [
  {
    title: 'Чернильная ночь',
    lines: ['Я растворяю звёзды в темноте,', 'и каждая строка — как капля света.', 'Луна качает лодку на воде,', 'а сердце ждёт несбывшегося лета.'],
  },
  {
    title: 'Гадание',
    lines: ['Раскинь мне карты на ладонь,', 'где судьбы пишутся водою.', 'Зажги в груди живой огонь —', 'и я пойду вслед за тобою.'],
  },
  {
    title: 'Серебро',
    lines: ['В моих ладонях — лунное литьё,', 'осколки снов и тихих обещаний.', 'Я отдаю тебе своё', 'из недосказанных молчаний.'],
  },
];
    ...
    <div className="min-h-screen text-foreground overflow-x-hidden">
      ...
      <Section id="poems">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {POEMS.map((p, i) => (
            <article key={i} className="glass rounded-lg p-8 hover:border-gold/50 transition-all hover:-translate-y-2 duration-300">
              <span className="text-gold text-2xl">❝</span>
              <h3 className="font-display text-2xl text-gold mb-4">{p.title}</h3>
              <div className="space-y-1">
                {p.lines.map((l, j) => (
                  <p key={j} className="font-serif text-lg italic text-foreground/85 leading-relaxed">{l}</p>
                ))}
              </div>
            </article>
          ))}
